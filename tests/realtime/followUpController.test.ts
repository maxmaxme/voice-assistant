...
describe('FollowUpController window request', () => {
  it('takeWindowRequest consumes the deferred request exactly once', () => {
    const c = new FollowUpController('sess1', 10_000);
    c.requestWindow();
...
describe('FollowUpController watchdog', () => {
  ...
  it('derives the deadline from the configured window, not a hardcoded 12s', () => {
    vi.useFakeTimers();
    // Admin set requestFollowUpMs above the firmware default — the watchdog
    // must not fire a false "user did not respond" at the old 12s mark.
    const c = new FollowUpController('sess1', 15_000);
    const logs = captureLogs();
    try {
      c.armWatchdog();
      vi.advanceTimersByTime(12_001);
