...
**Monitoring stack.** Same compose file also runs **Netdata** (metrics + alerts) and **Dozzle** (docker log viewer), both sitting on an internal `monitoring` bridge network with no host ports of their own. A tiny **Caddy** reverse proxy (`monitoring-proxy` service) is the only externally reachable entry point — it publishes `:19999` (→ netdata) and `:8888` (→ dozzle) with HTTP basic auth, credentials from `MONITOR_USER` + `MONITOR_PASSWORD_HASH` (bcrypt, generated once via `caddy hash-password`) in `.env`. Caddyfile is bind-mounted from `deploy/monitoring/Caddyfile`. A one-shot `netdata-init` busybox service writes a minimal `health_alarm_notify.conf` into the netdata config volume on every `docker compose up`, wiring Telegram alerts to the same `TELEGRAM_BOT_TOKEN` / `TELEGRAM_CHAT_ID` the assistant already uses — no manual setup. Tuning thresholds, password rotation, and security notes in `docs/monitoring-setup.md`.
