...
### Долгосрочная память (out of scope на старте)

Профиль пользователя, предпочтения, факты между днями — отдельный модуль `memory`
с интерфейсом `recall(query)` / `remember(fact)`. Добавится позже как ещё один адаптер,
если появится реальная потребность. Сейчас не строим.
